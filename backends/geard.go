...
import (
	...
	remote "github.com/openshift/geard/containers/http/remote"
	gclient "github.com/openshift/geard/http/client"
	"github.com/openshift/geard/jobs"
	"github.com/openshift/geard/port"
)

func Geard() engine.Installer {
	return &geard{}
}
...
func (s *geard) Install(eng *engine.Engine) error {
	eng.Register("geard", func(job *engine.Job) engine.Status {
		...
		job.Eng.Register("create", func(job *engine.Job) engine.Status {
			...
			req := &remote.HttpInstallContainerRequest{}
		...
		job.Eng.Register("start", func(job *engine.Job) engine.Status {
			...
			id := job.Args[0]
			req := &remote.HttpStartContainerRequest{}
		...
		job.Eng.Register("stop", func(job *engine.Job) engine.Status {
			...
			id := job.Args[0]
			req := &remote.HttpStopContainerRequest{}
