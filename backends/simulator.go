...
func Simulator() beam.Sender {
	...
	backend.OnSpawn(beam.Handler(func(ctx *beam.Message) error {
		s := &simulator{ctx.Args, &beam.Object{ctx.Ret}, beam.NewServer()}
		s.Server.OnAttach(beam.Handler(s.attach))
		s.Server.OnStart(beam.Handler(s.start))
		s.Server.OnLs(beam.Handler(s.ls))
		s.Server.OnSpawn(beam.Handler(s.spawn))
		_, err := ctx.Ret.Send(&beam.Message{Verb: beam.Ack, Ret: s.Server})
		return err
	}))
	return backend
}
