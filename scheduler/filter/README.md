...
As you can see here, the containers were only scheduled on nodes with the redis image alreayd pulled.
...
* `constraint:name!=/node-[01]-id/` will match all nodes, except those with ids `node-0-id` and `node-1-id`.
