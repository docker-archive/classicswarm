...
// Taken from Docker's Machine drivers/virtualbox/network.go
...
func (n *hostOnlyNetwork) Save() error {
	if n.IPv4.IP != nil && n.IPv4.Mask != nil {
		if err := exec.Command(VBOX_MANAGE, "hostonlyif",
			"ipconfig", n.Name,
			"--ip", n.IPv4.IP.String(),
			"--netmask", net.IP(n.IPv4.Mask).String()).Run(); err != nil {
			return err
		}
	}
	...
	if n.IPv6.IP != nil && n.IPv6.Mask != nil {
		...
		if err := exec.Command(VBOX_MANAGE, "hostonlyif",
			"ipconfig", n.Name,
			"--ipv6", n.IPv6.IP.String(),
			"--netmasklengthv6", fmt.Sprintf("%d", prefixLen)).Run(); err != nil {
			return err
		}
	}

	if n.DHCP {
		exec.Command(VBOX_MANAGE, "hostonlyif",
			"ipconfig", n.Name, "--dhcp").Run() // not implemented as of VirtualBox 4.3
	}
...
func createHostonlyNet() (*hostOnlyNetwork, error) {
	out, err := exec.Command(VBOX_MANAGE, "hostonlyif", "create").Output()
	if err != nil {
		return nil, err
	}
...
func listHostOnlyNetworks() (map[string]*hostOnlyNetwork, error) {
	out, err := exec.Command(VBOX_MANAGE, "list", "hostonlyifs").Output()
	if err != nil {
		return nil, err
	}
...
func addDHCPServer(kind, name string, d dhcpServer) error {
	...
	return exec.Command(VBOX_MANAGE, args...).Run()
}
...
func getDHCPServers() (map[string]*dhcpServer, error) {
	out, err := exec.Command(VBOX_MANAGE, "list", "dhcpservers").Output()
	if err != nil {
		return nil, err
	}
	s := bufio.NewScanner(strings.NewReader(string(out)))
